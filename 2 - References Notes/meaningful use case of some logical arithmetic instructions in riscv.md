...
But when the value in register is negative, `srai` doesn't means divide the value by $2$, which means we should take extra efforts to implement division instruction.